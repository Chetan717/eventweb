...
function UserDashboard() {
  ...
  console.log(allEvents,filterOptions?.category,filterOptions?.dateRange)
  ...
  useEffect(() => {
    const newFilteredEvents = allEvents.filter((event) => {
      ...
      const dateString = filterOptions?.dateRange;
      const dateParts = dateString.split("-");
      const formattedDate = `${parseInt(dateParts[2])}/${parseInt(dateParts[1])}/${dateParts[0]}`;
      if ( filterOptions?.dateRange && event?.date !== formattedDate) {
        return false;
      }

      // Check if category filter matches
      if (filterOptions.category && event.Type !== filterOptions.category) {
        return false;
      }
